...
- Fuzz and finish short-read/error propagation in the legacy AIFF importer before accepting untrusted files as a hardened production claim. DXM has bounded-reader coverage; S3M, DIGI, MOD, BEM, IT, XM, BRR, IFF, and WAV now reject truncated or structurally invalid payloads. WAV channel selection is also consistent across 8/16/24/32-bit PCM and 32/64-bit floating-point input, with planar stereo output when requested.
