...
cat >"$BUILDDIR/ft2-dxm.desktop" <<EOF
[Desktop Entry]
Name=ft2-dxm
Exec=ft2-dxm
Icon=ft2-dxm
...
ROOTDIR="$PWD"
cd "$BUILDDIR"
"./$LINUXDEPLOY" --appdir ft2-dxm.AppDir --output appimage --icon-file "$ROOTDIR/src/gfxdata/icon/ft2-dxm.png" --icon-filename "ft2-dxm" --desktop-file "ft2-dxm.desktop" || exit 1

echo Done. The AppImage can be found at \'$BUILDDIR/ft2-dxm-$(uname -m).AppImage\' if everything went well.
