...
void Dx7Note::init(const uint8_t patch[156], int midinote, int velocity, int channel, const Controllers *ctrls) {
    ...
    {
        // Clamp feedback to valid range [0 .. FEEDBACK_BITDEPTH] and treat feedback==0
        // as maximum feedback (FEEDBACK_BITDEPTH). This aligns behavior with DX7
        // semantics where a zero feedback value corresponds to maximum shift.
        int feedback = patch[135];
        if (feedback < 0) feedback = 0;
        if (feedback > FEEDBACK_BITDEPTH) feedback = FEEDBACK_BITDEPTH;
        // Treat feedback==0 as FEEDBACK_BITDEPTH so fb_shift_ stays in [0..FEEDBACK_BITDEPTH].
...
//    printf("[DX7NOTE] compute ENTRY: this=%p ctrls=%p\n", (void*)this, (void*)ctrls);
//    if (!ctrls) {
//        printf("[DX7NOTE] ERROR: ctrls is NULL!\n");
//        return;
//    }
//    printf("[DX7NOTE] compute: core=%p\n", (void*)ctrls->core);
//    if (!ctrls->core) {
//        printf("[DX7NOTE] ERROR: ctrls->core is NULL!\n");
//        return;
//    }
    if (fb_shift_ < 0 || fb_shift_ > FEEDBACK_BITDEPTH) {
//        printf("[DX7NOTE] Warning: fb_shift_ out of range (%d). Clamping to [0..%d]\n", fb_shift_, FEEDBACK_BITDEPTH);
        if (fb_shift_ < 0) fb_shift_ = 0;
        if (fb_shift_ > FEEDBACK_BITDEPTH) fb_shift_ = FEEDBACK_BITDEPTH;
    }



    // Clear the feedback buffer to prevent state leakage between compute calls,
    // especially when processing in small chunks. This is a brute-force fix
    // for potential audio glitches.
    fb_buf_[0] = 0;
    fb_buf_[1] = 0;

    // Basic null checks for controllers/core to avoid crashes and give helpful logs.
    if (!ctrls) {
//        printf("[DX7NOTE] Error: controllers pointer is NULL in compute()\n");
        return;
    }
    if (!ctrls->core) {
//       printf("[DX7NOTE] Error: controllers->core is NULL in compute()\n");
        return;
    }

    // Small sanity checks for LFO values: log unusual values which might indicate
    // upstream corruption. Do not bail out for moderately strange values, just log.
//    if (lfo_delay < 0 || lfo_delay > (1<<30)) {
//        printf("[DX7NOTE] Warning: lfo_delay out of expected range: %d\n", lfo_delay);
//   }

    // ==== PITCH ====
    //
//    printf("[DX7NOTE] compute: fb_shift_=%d pitchmoddepth_=%d pitchmodsens_=%d\n",
//           fb_shift_, pitchmoddepth_, pitchmodsens_);
//    printf("[DX7NOTE] compute: about to call pitchenv_.getsample()\n");
    int32_t pes = pitchenv_.getsample();
//    printf("[DX7NOTE] compute: pitchenv_.getsample()=%d\n", pes);

//    printf("[DX7NOTE] compute: about to compute pmod_1\n");///
    ...
    if (pb != 0) {
        if (ctrls->values_[kControllerPitchStep] == 0) {
            // Fixed-point arithmetic for pitch bend.
            // (pb << 11) is Q11 format. We'll use Q24 for intermediate calculations.
            // The result is scaled by 2^11 to match the original float calculation's implicit scaling.
            if( pb >= 0 )
                pb = (int32_t)(((int64_t)(pb << 11) * ctrls->values_[kControllerPitchRangeUp]) / 12);
            else
                pb = (int32_t)(((int64_t)(pb << 11) * ctrls->values_[kControllerPitchRangeDn]) / 12);
        } else {
    ...
    if( ctrls->mpeEnabled )
    {
        // Fixed-point arithmetic for MPE pitch bend.
        // (mpePitchBend-0x2000) << 11 is Q11 format.
        int d = (int32_t)(((int64_t)((mpePitchBend-0x2000) << 11) * ctrls->mpePitchBendRange) / 12);
    ...
    if( ! tuning_state_->is_standard_tuning() && pb != 0 )
    {
        ...
        // Fixed-point arithmetic for non-standard tuning pitch bend.
        // pb is in Q11 format (from previous calculations). We use Q16 for fractional notes.
        // (pb >> 11) is the integer bend in semitones.
        // ((pb & 0x7FF) * 12) / 8192 is the fractional part in Q16.
        int32_t notesTuned_int = pb >> 11;
        int32_t notesTuned_frac = ((pb & 0x7FF) * 12 * 65536) / 8192;

        int32_t floorNote = notesTuned_int;
        int32_t frac = notesTuned_frac; // Q16

        // Interpolate between log frequencies. The tuning functions return Q24 format.
        // We'll do the interpolation in Q24.
        int64_t log1 = tuning_state_->midinote_to_logfreq(playingMidiNote + floorNote);
        int64_t log2 = tuning_state_->midinote_to_logfreq(playingMidiNote + floorNote + 1);
        int64_t targetLog = (log1 * (65536 - frac) + log2 * frac) / 65536; // Q24
        int64_t baseLog = tuning_state_->midinote_to_logfreq(playingMidiNote); // Q24
        int64_t newpb = targetLog - baseLog; // Q24

        // The original pb was in Q11. We need to convert newpb from Q24 to Q11.
        pb = (int32_t)(newpb >> (24 - 11));
    }
    ...
    for (int op = 0; op < 6; op++) {
        if ( ctrls->opSwitch[op] == '0' )  {
            ...
            if (ampmodsens_[op] != 0) {
                ...
                // Optimized to avoid float conversion. exp(x) where x is small.
                // We use a lookup table approximation for exp(x) to maintain performance.
                // The original float calculation was: exp(sensamp/262144 * 0.07 + 12.2)
                // Let's approximate this. The range of sensamp is limited.
                // For now, we keep the float but ensure it's a single conversion to reduce overhead.
                // A full fixed-point exp() would be more involved.
                float pt_f = expf(((float)sensamp)/262144.0f * 0.07f + 12.2f);
                uint32_t pt = (uint32_t)pt_f;
