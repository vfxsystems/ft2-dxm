...
static uint8_t getFT2PalNrFromPixelWithKey(uint32_t pixel32)
{
	if (pixel32 == 0x00FF00)
		return PAL_TRANSPR; // transparency key color

	return (uint8_t)getFT2PalNrFromPixel(pixel32);
}
...
uint8_t *ft2_bmp_decode_rle4_to_pal(const uint8_t *src, int32_t *out_w, int32_t *out_h)
{
	...
	const int32_t width = hdr->biWidth;
	const int32_t height = (hdr->biHeight < 0) ? -hdr->biHeight : hdr->biHeight;
	if (width <= 0 || height <= 0)
		return NULL;

	uint8_t *outData = (uint8_t *)malloc(width * height * sizeof (uint8_t));
	if (outData == NULL)
		return NULL;

#ifdef _DEBUG
	const uint8_t *allocEnd = outData + (width * height);
#endif
	...
	// pre-fill image with first palette color
	palIdx = getFT2PalNrFromPixelWithKey(pal[0]);
	for (i = 0; i < width * height; i++)
		outData[i] = palIdx;

	const int32_t lineEnd = width;
	const uint8_t *src8 = pData;
#ifdef _DEBUG
	const uint8_t *src8End = src8 + hdr->biSizeImage;
#endif
	...
	while (true)
	{
		...
		if (byte == 0) // escape control
		{
			...
			else // absolute bytes
			{
				...
				for (i = 0; i < len; i++)
				{
					...
					CHECK_DST8_BOUNDARY_X
					tmp8[x++] = getFT2PalNrFromPixelWithKey(pal[palIdx >> 4]);

					if (x < lineEnd)
					{
						CHECK_DST8_BOUNDARY_X
						tmp8[x++] = getFT2PalNrFromPixelWithKey(pal[palIdx & 0xF]);
					}
				}
		...
		else
		{
			...
			pal1 = getFT2PalNrFromPixelWithKey(pal[palIdx >> 4]);
			pal2 = getFT2PalNrFromPixelWithKey(pal[palIdx & 0x0F]);
