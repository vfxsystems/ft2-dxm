...
static bool testEditKeys(SDL_Scancode scancode, SDL_Keycode keycode)
{
	...
	if (cursor.ch < MAX_STEREO_PAIRS && editor.macroMode[cursor.ch] && macroPattern[curPattern] != NULL)
		mn = &macroPattern[curPattern][(row * MAX_STEREO_PAIRS) + cursor.ch];
...
void recordNote(uint8_t noteNum, int8_t vol) // directly ported from the original FT2 code - what a mess, but it works...
{
	...
	else
	{
		...
		{
#ifdef HAS_MIDI
			playTone(c, channel[c].instrNum, NOTE_OFF, vol, midi.currMIDIVibDepth, midi.currMIDIPitch);
#else
			playTone(c, channel[c].instrNum, NOTE_OFF, vol, 0, 0);
#endif
		}
...
static void copyMacroNote(const macroNote_t *src, macroNote_t *dst)
{
	*dst = *src;
}

static void pasteMacroNote(const macroNote_t *src, macroNote_t *dst)
{
	*dst = *src;
}
...
void cutTrack(void)
{
	...
	if (cursor.ch < MAX_STEREO_PAIRS && editor.macroMode[cursor.ch] && macroPattern[curPattern] != NULL)
	{
		for (int16_t i = 0; i < numRows; i++)
		{
			macroNote_t *mn = &macroPattern[curPattern][(i * MAX_STEREO_PAIRS) + cursor.ch];
			mn->slot = 0xFF;
			mn->mode = 0xFF;
			mn->value = 0x00;
		}
...
void cutPattern(void)
{
	...
	if (macroPattern[curPattern] != NULL)
	{
		for (int16_t x = 0; x < MAX_STEREO_PAIRS; x++)
		{
			...
			for (int16_t i = 0; i < numRows; i++)
			{
				macroNote_t *mn = &macroPattern[curPattern][(i * MAX_STEREO_PAIRS) + x];
				mn->slot = 0xFF;
				mn->mode = 0xFF;
				mn->value = 0x00;
			}
		...
		for (int16_t x = 0; x < MAX_STEREO_PAIRS; x++)
		{
			...
			for (int16_t i = 0; i < numRows; i++)
				pasteMacroNote(&macroPtnCopyBuff[(i * MAX_STEREO_PAIRS) + x],
				               &macroPattern[curPattern][(i * MAX_STEREO_PAIRS) + x]);
		}
	}
	resumeMusic();
	...
	if (p != NULL && markY1 >= 0 && markX1 >= 0 && markX2 >= 0 && markY2 >= 0)
	{
		...
		for (int32_t x = markX1; x <= markX2; x++)
		{
			for (int32_t y = markY1; y < markY2; y++)
			{
				...
				if (x < MAX_STEREO_PAIRS && editor.macroMode[x] && macroPattern[curPattern] != NULL)
				{
					macroNote_t *mn = &macroPattern[curPattern][(y * MAX_STEREO_PAIRS) + x];
					mn->slot = 0xFF;
					mn->mode = 0xFF;
					mn->value = 0x00;
				}
