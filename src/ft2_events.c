	...
	if (oneInstHandle != NULL)
			...
			if (!backupMadeAfterCrash)
		{
			if (getDiskOpModPath() != NULL && UNICHAR_CHDIR(getDiskOpModPath()) == 0)
			{
				...
				{
					sprintf(fileName, "backup%03d.dxm", (int32_t)i);
	...
	if (signal == 15)
			...
			if (!backupMadeAfterCrash)
		{
			if (getDiskOpModPath() != NULL && UNICHAR_CHDIR(getDiskOpModPath()) == 0)
			{
				...
				for (i = 1; i < BACKUP_FILES_TO_TRY; i++)
				{
					sprintf(fileName, "backup%03d.dxm", i);
					if (stat(fileName, &statBuffer) != 0)
						break; // filename OK
				}
...
static void handleSDLEvents(void)
{
	...
	while (SDL_PollEvent(&event))
	{
		...
		else if (event.type == SDL_MOUSEMOTION)
		{
			...
			armMacGhostMouseCursorFix();
#endif
			if (ui.synthEditorShown && (event.motion.state & SDL_BUTTON_LMASK))
			{
				if (editor.curInstr >= 1 && editor.curInstr <= MAX_INST && instr[editor.curInstr] != NULL)
				{
					instr_t *ins = instr[editor.curInstr];
					int32_t mx = event.motion.x;
					int32_t my = event.motion.y;
					if (video.fullscreen)
					{
						mx -= video.renderX;
						my -= video.renderY;
