...
// new macro: number of stereo pairs (each pair = 2 tracker channels)
#define MAX_STEREO_PAIRS (MAX_CHANNELS / 2)
// helper: map tracker channel (0..31) to stereo pair index (0..15)
// Changed: now maps channels 0-15 directly to pairs 0-15 (no grouping)
#define CHANNEL_TO_PAIR_IDX(ch) ((ch) < 16 ? (ch) : ((ch) >> 1))

// DXM format flag for stereo sample handling
// isDXMFormat is defined in ft2_module_loader.c and used for loading only
