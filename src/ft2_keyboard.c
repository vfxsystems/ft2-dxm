...
void keyDownHandler(SDL_Scancode scancode, SDL_Keycode keycode, bool keyWasRepeated)
{
    ...
    {
        popupListHandleKey(scancode, true);
        return;
    }
	...
	if (ui.sysReqShown && !ui.mixerScreenShown)
	{
		/* Real modal system request (not mixer). Block further key handling */
...
static void handleKeys(SDL_Keycode keycode, SDL_Scancode scanKey)
{
	...
	switch (keycode)
	{
		...
		case SDLK_DELETE: // non-FT2 addition
		{
			...
			{
				// Check if we should delete a slice marker instead
				if (getSliceMarkersVisible() && selectedSliceMarker >= 0)
				{
					handleSliceMarkerDelete();
				}
				else
				{
					sampCut();
		...
		case SDLK_SPACE:
		{
            // Always send panic to synth engines to kill hanging voices
            ft2_synth_panic();
            ft2_dx_panic();
