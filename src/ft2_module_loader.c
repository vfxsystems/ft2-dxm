...
{
	"dxm", "xm",
	// "ft", "nst", "stk", "mod", "s3m", "stm", "fst", "digi", "bem", "it", // legacy removed
	"END_OF_LIST"
};
...
static bool doLoadMusic(bool externalThreadFlag)
{
	...
	// Reset DXM format flag at start of loading
	isDXMFormat = false;
	...
	switch (format)
	{
		case FORMAT_DXM: 
			isDXMFormat = true;
			moduleLoaded = loadDXM(f, filesize); 
			break;
		case FORMAT_XM: 
			isDXMFormat = false;
			moduleLoaded = loadXM(f, filesize); 
			// XM loading is for import only - convert to DXM format internally
			if (moduleLoaded) {
				// Mark all instruments as DXM format for proper stereo handling
				for (int i = 1; i <= 128; i++) {
					if (instrTmp[i] != NULL) {
						instrTmp[i]->isDXMInstrument = true;
					}
				}
			}
			break;
	...
	// Reset DXM format flag after loading
	isDXMFormat = false;
