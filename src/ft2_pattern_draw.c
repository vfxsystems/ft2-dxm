...
{
	const int32_t cursorObj = (cursor.object == CURSOR_EFX3) ? CURSOR_EFX0 : cursor.object;
	const int32_t tabOffset = (config.ptnShowVolColumn * 32) + (columnModeTab[ui.numChannelsShown-1] * 8) + cursorObj;

	int32_t xPos = pattCursorXTab[tabOffset];
	int32_t width = pattCursorWTab[tabOffset];
	...
	case CURSOR_EFX3:
	{
		...
		if (config.ptnShowVolColumn)
			charW = (ui.numChannelsShown <= 6) ? FONT4_CHAR_W : FONT3_CHAR_W;
		else
			charW = (ui.numChannelsShown <= 4) ? FONT5_CHAR_W : (ui.numChannelsShown <= 8) ? FONT4_CHAR_W : FONT3_CHAR_W;
		xPos -= charW;
		width = charW;
		break;
	}
	default:
		/* FX stays at original position */
		break;
	}
...
static void showMacroEfx(uint32_t xPos, uint32_t yPos, const macroNote_t *mn, uint32_t color)
{
	uint8_t fontType, charW;
	...
	const uint32_t xPrefix = xPos - charW;
	if (mn == NULL || mn->slot == 0xFF)
	{
		pattCharOut(xPrefix,            yPos, 39, fontType, color);
		pattCharOut(xPos,               yPos, 39, fontType, color);
		pattCharOut(xPos +  charW,      yPos, 39, fontType, color);
		pattCharOut(xPos + (charW * 2), yPos, 39, fontType, color);
		return;
	}

	const uint8_t mode = (mn->mode == 0xFF) ? 39 : (mn->mode & 0x0F);
	pattCharOut(xPrefix,            yPos, mn->slot & 0x0F,           fontType, color);
	pattCharOut(xPos,               yPos, mode,                      fontType, color);
	pattCharOut(xPos +  charW,      yPos, mn->value >> 4,            fontType, color);
	pattCharOut(xPos + (charW * 2), yPos, mn->value & 0x0F,          fontType, color);
}
	...
	*/
	...
	for (int32_t i = 0; i < rowsOnScreen; i++)
	{
		if (row >= 0)
		{
			...
			{
				const int chAbs = ui.channelOffset + j;
				drawNote(xPos, textY, p->note, color);
				drawInst(xPos, textY, p->instr, color);
				drawVolEfx(xPos, textY, p->vol, color);
				if (chAbs < MAX_STEREO_PAIRS && editor.macroMode[chAbs])
				{
					const macroNote_t *mn = NULL;
					if (macroPattern[currPattern] != NULL)
						mn = &macroPattern[currPattern][(uint32_t)row * MAX_STEREO_PAIRS + chAbs];
					showMacroEfx(xPos, textY, mn, color);
				}
				else
				{
