...
bool allocatePattern(uint16_t pattNum) // for tracker use only, not in loader!
{
	...
	{
		macroPattern[pattNum] = (macroNote_t *)malloc(MAX_PATT_LEN * MAX_STEREO_PAIRS * sizeof (macroNote_t));
		if (macroPattern[pattNum] == NULL)
		{
			if (audioWasntLocked)
				unlockAudio();
		...
		memset(macroPattern[pattNum], 0xFF, MAX_PATT_LEN * MAX_STEREO_PAIRS * sizeof (macroNote_t));
	}
...
void killPatternIfUnused(uint16_t pattNum) // for tracker use only, not in loader!
{
	...
	if (patternEmpty(pattNum))
	{
		...
		if (macroPattern[pattNum] != NULL)
		{
			free(macroPattern[pattNum]);
			macroPattern[pattNum] = NULL;
		}
	}
...
void cursorChannelLeft(void)
{
	...
	cursor.object = (cursor.ch < MAX_STEREO_PAIRS && editor.macroMode[cursor.ch]) ? CURSOR_EFX3 : CURSOR_EFX2;
}
...
{
	if (cursor.ch < MAX_STEREO_PAIRS && editor.macroMode[cursor.ch])
	{
		if (cursor.object == CURSOR_EFX0)
		{
			cursor.object = CURSOR_EFX3;
			ui.updatePatternEditor = true;
			return;
		}
		else if (cursor.object == CURSOR_EFX3)
		{
			cursor.object = config.ptnShowVolColumn ? CURSOR_VOL2 : CURSOR_INST2;
			ui.updatePatternEditor = true;
			return;
		}
	}
...
void cursorRight(void)
{
	if (cursor.ch < MAX_STEREO_PAIRS && editor.macroMode[cursor.ch])
	{
		if (cursor.object == (config.ptnShowVolColumn ? CURSOR_VOL2 : CURSOR_INST2))
		{
			cursor.object = CURSOR_EFX3;
			ui.updatePatternEditor = true;
			return;
	...
	{
		const int32_t maxObj = CURSOR_EFX2;
		if (cursor.object > maxObj)
		{
			cursor.object = CURSOR_NOTE;
			cursorChannelRight();
		}
