...
// Forward declarations for radio button handlers and updateWindowTitle
...
radioButton_t radioButtons[NUM_RADIOBUTTONS] =
{
	...
	*/
	...
	//x, y,  w,   group,                      funcOnUp
	{ 4, 100, 40, RB_GROUP_DISKOP_MOD_SAVEAS, rbDiskOpModSaveDxm }, // DXM
//	{ 4, 114, 33, RB_GROUP_DISKOP_MOD_SAVEAS, rbDiskOpModSaveXm },  // XM
	{ 4, 128, 40, RB_GROUP_DISKOP_MOD_SAVEAS, rbDiskOpModSaveWav }, // WAV

	// INSTRUMENT SAVE AS FORMATS
	//x, y,   w,  group,                      funcOnUp
	{ 4, 100, 29, RB_GROUP_DISKOP_INS_SAVEAS, rbDiskOpInsSaveXi  },
	{ 4, 114, 40, RB_GROUP_DISKOP_INS_SAVEAS, rbDiskOpInsSaveDxi },
	...
	//x,   y,  w,  group,                        funcOnUp
	{ 190, 95, 45, RB_GROUP_WAV_RENDER_BITDEPTH, rbWavRenderBitDepth16 },
	{ 240, 95, 45, RB_GROUP_WAV_RENDER_BITDEPTH, rbWavRenderBitDepth32 },

	// WAV RENDERER TARGET
	//x,   y,  w,  group,                        funcOnUp
	{ 90, 116, 45, RB_GROUP_WAV_RENDER_TARGET, rbWavRenderTargetFile },
	{ 90, 129, 45, RB_GROUP_WAV_RENDER_TARGET, rbWavRenderTargetSlot }
};
...
void checkRadioButton(uint16_t radioButtonID)
{
	...
	for (uint16_t i = 0; i < NUM_RADIOBUTTONS; i++, radioButton++)
	{
		if (radioButton->group == testGroup && radioButton->state == RADIOBUTTON_CHECKED)
		{
			...
			drawRadioButton(i);
			/* don't break here – there could be more than one previously checked */
		}
	}
...
void showRadioButtonGroup(uint16_t radioButtonGroup)
{
	...
	/* first pass: make sure at most one button in group is checked */
	radioButton_t *rb = radioButtons;
	for (uint16_t i = 0; i < NUM_RADIOBUTTONS; i++, rb++)
	{
		if (rb->group != radioButtonGroup)
			continue;
		...
		if (rb->state == RADIOBUTTON_CHECKED)
		{
			if (!foundChecked)
			{
				foundChecked = true; /* keep first */
			}
			else
			{
				rb->state = RADIOBUTTON_UNCHECKED; /* clear duplicates */
			}
		}
	}
...
{
	if (ui.sysReqShown && !ui.wavRendererShown)
		return false;
