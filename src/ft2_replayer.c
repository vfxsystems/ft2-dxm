...
macroNote_t *macroPattern[MAX_PATTERNS] = { NULL };

#define MACRO_MODE_RAW    0x01
#define MACRO_MODE_SMOOTH 0x02
...
static uint16_t synthMidiLastTick[MAX_CHANNELS];
...
void resetChannels(void)
{
	...
	memset(editor.macroMode, 0, sizeof (editor.macroMode));
...
void calcReplayerVars(int32_t audioFreq)
{
	...
	// Use higher precision for timing calculations to prevent choppy audio at lower sample rates
	const double highPrecisionScale = 65536.0; // 16-bit fractional precision
	...
	for (int32_t bpm = MIN_BPM; bpm <= MAX_BPM; bpm++)
	{
		...
		// Use higher precision fractional calculation
		double dSamplesPerTickInt;
		double dSamplesPerTickFrac = modf(dSamplesPerTick * highPrecisionScale, &dSamplesPerTickInt);
		
		audio.samplesPerTickIntTab[i] = (uint32_t)(dSamplesPerTickInt / highPrecisionScale);
		audio.samplesPerTickFracTab[i] = (uint64_t)((dSamplesPerTickFrac / highPrecisionScale * BPM_FRAC_SCALE) + 0.5);

		// BPM Hz -> tick length for performance counter (syncing visuals to audio)
		double dTimeInt;
		double dTimeFrac = modf(editor.dPerfFreq / dBpmHz * highPrecisionScale, &dTimeInt);
		
		audio.tickTimeIntTab[i] = (uint32_t)(dTimeInt / highPrecisionScale);
		audio.tickTimeFracTab[i] = (uint64_t)((dTimeFrac / highPrecisionScale * TICK_TIME_FRAC_SCALE) + 0.5);
	}
}
...
void triggerNote(uint8_t note, uint8_t efx, uint8_t efxData, channel_t *ch)
{
    ...
    if (note == NOTE_OFF)
    {
        // Check if instrument uses Tunefish4 or Dexed synth and handle note-off
...
/* DXM macro Mxy: X=macro index 0-F, Y=value 0-F (nibble). Legacy 4-bit mode. */
static void setMacroValue(channel_t *ch, uint8_t param)
{
	const uint8_t macroIdx = param >> 4;
	const uint8_t macroVal = param & 0x0F;
	if (macroIdx < 16)
	{
		ch->macroIndexLatch = macroIdx;
		ch->macroVal[macroIdx] = (uint8_t)(macroVal * 17); // 0..15 -> 0..255
		ch->macroMode[macroIdx] = 0;
		printf("[MACRO] setMacroValue: idx=%u val=%u\n", macroIdx, macroVal);
	}
}

/* DXM macro Zx: select macro index (0-F). */
static void setMacroIndex(channel_t *ch, uint8_t param)
{
	const uint8_t macroIdx = param & 0x0F;
	if (macroIdx < 16)
	{
		ch->macroIndexLatch = macroIdx;
	}
...
/* DXM macro Yyy: set macro value 00-FF using latched macro index. */
static void setMacroValue8(channel_t *ch, uint8_t param)
{
	const uint8_t macroIdx = ch->macroIndexLatch & 0x0F;
	if (macroIdx < 16)
	{
		ch->macroVal[macroIdx] = param;
		ch->macroMode[macroIdx] = 0;
		printf("[MACRO] setMacroValue8: idx=%u val=%u\n", macroIdx, param);
	}
}

/* DXM macro Nyy: set raw macro value 00-FF using latched macro index. */
static void setRawMacroValue8(channel_t *ch, uint8_t param)
{
	const uint8_t macroIdx = ch->macroIndexLatch & 0x0F;
	if (macroIdx < 16)
	{
		ch->macroVal[macroIdx] = param;
		ch->macroMode[macroIdx] = MACRO_MODE_RAW;
		printf("[MACRO] setRawMacroValue8: idx=%u val=%u\n", macroIdx, param);
	}
}

/* DXM macro Uyy: set smooth macro value 00-FF using latched macro index. */
static void setSmoothMacroValue8(channel_t *ch, uint8_t param)
{
	const uint8_t macroIdx = ch->macroIndexLatch & 0x0F;
	if (macroIdx < 16)
	{
		ch->macroVal[macroIdx] = param;
		ch->macroMode[macroIdx] = MACRO_MODE_SMOOTH;
		printf("[MACRO] setSmoothMacroValue8: idx=%u val=%u\n", macroIdx, param);
	}
}
...
{
    if (ch == NULL || ch->instrPtr == NULL)
        return;

    instr_t *ins = ch->instrPtr;

    const int chIndex = (int)(ch - channel);
    const int pairIdx = CHANNEL_TO_PAIR_IDX(chIndex);

    for (int m = 0; m < 16; m++)
    {
        const uint8_t rawVal     = ch->macroVal[m];      // 0..255
        const uint8_t prevVal    = ch->prevMacroVal[m];  // Previous value
        const uint8_t mode       = ch->macroMode[m];
        const uint8_t targetType = ins->macroTargetType[m];
        const uint16_t paramID   = ins->macroParamID[m];
        const uint8_t curve      = ins->macroScale[m];

        const bool isRaw = (mode & MACRO_MODE_RAW) != 0;
        const bool isSmooth = (mode & MACRO_MODE_SMOOTH) != 0;

        const bool smoothActive = isSmooth && (fabsf(ch->macroSmoothCurrent[m] - ch->macroSmoothTarget[m]) > 0.0005f);
        if ((rawVal != prevVal || smoothActive) && targetType != MACRO_TARGET_NONE)
        {
            float norm;
            ...
            if (isRaw) {
                // Raw mode: direct 0-255 to 0.0-1.0 mapping (no curve processing)
                norm = rawVal / 255.0f;
                scaled = norm;
            } else if (isSmooth) {
                // Smooth mode: interpolate to target value
                const float targetNorm = rawVal / 255.0f;
                ch->macroSmoothTarget[m] = targetNorm;
                
                // Interpolate current value towards target (simple linear interpolation)
                const float smoothRate = 0.1f; // Adjust for faster/slower interpolation
                ch->macroSmoothCurrent[m] += (ch->macroSmoothTarget[m] - ch->macroSmoothCurrent[m]) * smoothRate;
                
                // Apply curve to interpolated value
                norm = ch->macroSmoothCurrent[m];
                switch (curve)
                {
                    case MACRO_CURVE_LOG:   scaled = norm * norm; break;
                    case MACRO_CURVE_EXP:   scaled = norm * norm * norm; break;
                    case MACRO_CURVE_SQUARE:scaled = norm * norm; break;
                    case MACRO_CURVE_SQRT:  scaled = sqrtf(norm); break;
                    default:                scaled = norm; break;
                }
            } else {
                // Normal mode: apply curve processing
                norm = rawVal / 255.0f;
                switch (curve)
                {
                    case MACRO_CURVE_LOG:   scaled = norm * norm; break;
            ...
            {
                const uint8_t scope = DSP_MACRO_SCOPE(paramID);
                const uint8_t slot = DSP_MACRO_SLOT(paramID);
                const uint8_t pIdx = DSP_MACRO_PARAM(paramID);

                if (slot >= DSP_MAX_SLOTS)
                    continue;

                dspEffectInstance_t *eff = NULL;
                if (scope == DSP_MACRO_SCOPE_MASTER)
                    eff = &masterEffects[slot];
                else
                    eff = &stereoMixerCh[pairIdx].effects[slot];

                if (eff && eff->type != DSP_TYPE_NONE)
                {
                    int numParams = 0;
                    const dspParamInfo_t *pi = dspGetParamInfo(eff->type, &numParams);
                    if (pi && pIdx < numParams)
                    {
                        float *p = getParamPtr(eff, pIdx);
                        if (p)
                            *p = pi[pIdx].min + scaled * (pi[pIdx].max - pi[pIdx].min);
                    }
                }

                // Request DSP UI sync from main thread
                extern void requestDspUiSync(void);
                requestDspUiSync();
            }

            // Update previous value
            ch->prevMacroVal[m] = rawVal;
        }
    }
}
...
static const efxRoutine JumpTab_TickZero[36] =
{
	...
	dummy,			// K
	setEnvelopePos,		// L
	setMacroValue,		// M (legacy 4-bit)
	setRawMacroValue8,	// N
	dummy,			// O
	dummy,			// P
	dummy,			// Q
	dummy,			// R (reserved for retrig)
	dummy,			// S
	dummy,			// T
	setSmoothMacroValue8,	// U
	dummy,			// V
	dummy,			// W
	dummy,			// X
	setMacroValue8,		// Y
...
{
	const int chIndex = (int)(ch - channel);
	if (chIndex >= 0 && chIndex < MAX_STEREO_PAIRS && editor.macroMode[chIndex])
	{
		if (macroPattern[song.pattNum] != NULL)
		{
			const macroNote_t *mn = &macroPattern[song.pattNum][(song.row * MAX_STEREO_PAIRS) + chIndex];
			if (mn->slot != 0xFF && mn->slot < 16)
			{
				ch->macroVal[mn->slot] = mn->value;
				if (mn->mode == 1)
					ch->macroMode[mn->slot] = MACRO_MODE_RAW;
				else if (mn->mode == 2)
					ch->macroMode[mn->slot] = MACRO_MODE_SMOOTH;
				else
					ch->macroMode[mn->slot] = 0;
			}
		}
	}
...
void samp2Delta(int8_t *p, int32_t length, uint8_t smpFlags)
{
	...
	if (stereo)
	{
		...
		if (sample16Bit)
		{
			...
			for (int32_t i = 0; i < length; i++)
			{
				const int16_t oldS16L = p16L[i];
				const int16_t oldS16R = p16R[i];
				p16L[i] -= newS16L;
				p16R[i] -= newS16R;
				newS16L = oldS16L;
				newS16R = oldS16R;
		...
		else // 8-bit
		{
			...
			for (int32_t i = 0; i < length; i++)
			{
				const int8_t oldS8L = p8L[i];
				const int8_t oldS8R = p8R[i];
				p8L[i] -= newS8L;
				p8R[i] -= newS8R;
				newS8L = oldS8L;
				newS8R = oldS8R;
...
bool patternEmpty(uint16_t pattNum)
{
	...
	if (macroPattern[pattNum] != NULL)
	{
		const macroNote_t *m = macroPattern[pattNum];
		const uint32_t mLen = patternNumRows[pattNum] * MAX_STEREO_PAIRS;
...
void stopPlaying(void)
{
	...
	// Send MIDI note-off to synth engines before stopping playback
...
void playTone(uint8_t chNum, uint8_t insNum, uint8_t note, int8_t vol, uint16_t midiVibDepth, uint16_t midiPitch)
{
	...
	// Original sample-based processing
