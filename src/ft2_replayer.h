#ifndef FT2_REPLAYER_H
#define FT2_REPLAYER_H
...
typedef struct macroNote_t
{
	uint8_t slot;  // 0..15, 0xFF = empty
	uint8_t mode;  // 0=normal,1=raw,2=smooth, 0xFF = empty
	uint8_t value; // 0..255
}
#ifdef __GNUC__
__attribute__ ((packed))
...
typedef struct sample_t
{
    ...
    // New: Separate L/R buffers for stereo support
    int8_t *dataPtrL, *dataPtrR; // Left and Right channel data (8-bit or 16-bit cast)
    int8_t *origDataPtrL, *origDataPtrR; // For allocation/freeing

    // Deprecated: Old mono/interleaved pointer (to be removed after refactor)
	int8_t *dataPtr; // DEPRECATED: use dataPtrL/dataPtrR
	int8_t *origDataPtr; // DEPRECATED: use origDataPtrL/origDataPtrR
...
typedef struct channel_t
{
	...
	/* DXM: tracker macro values (00-FF) */
	uint8_t macroVal[16];
	uint8_t prevMacroVal[16]; /* Previous values for change detection */
	uint8_t macroMode[16]; /* bit0=raw, bit1=smooth */
	uint8_t macroIndexLatch; /* selected macro index (0-F) */
	float macroSmoothCurrent[16]; /* Current interpolated values for smooth mode */
	float macroSmoothTarget[16];  /* Target values for smooth interpolation */
