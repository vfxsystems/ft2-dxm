...
bool allocateSmpData(sample_t *s, int32_t length, bool sample16Bit, bool stereo)
{
	...
	if (s->origDataPtrL == NULL) {
		...
		s->origDataPtrR = NULL;
		s->dataPtrR = NULL;
		s->dataPtr = NULL; // deprecated
		s->origDataPtr = NULL; // deprecated
		return false;
	}
	s->dataPtrL = s->origDataPtrL + SMP_DAT_OFFSET;
	...
	if (stereo) {
		...
		s->dataPtr = s->dataPtrL; // alias for legacy routines
		s->origDataPtr = s->origDataPtrL;
	} else {
		s->origDataPtrR = NULL;
		s->dataPtrR = NULL;
		s->dataPtr = s->dataPtrL; // deprecated: mono compatibility
		s->origDataPtr = s->origDataPtrL;
	}
	return true;
}
...
bool reallocateSmpData(sample_t *s, int32_t length, bool sample16Bit)
{
	...
	if (stereo) {
		...
		// If previously stereo, free R
		if (s->origDataPtrR != NULL) {
			free(s->origDataPtrR);
			s->origDataPtrR = NULL;
			s->dataPtrR = NULL;
		}
		s->dataPtr = s->dataPtrL; // deprecated
		s->origDataPtr = s->origDataPtrL;
	}
	return true;
}
...
bool cloneSample(sample_t *src, sample_t *dst)
{
	...
	if (src == NULL) {
		...
		dst->origDataPtrL = dst->dataPtrL = NULL;
		dst->origDataPtrR = dst->dataPtrR = NULL;
		dst->origDataPtr = dst->dataPtr = NULL; // deprecated
		dst->isFixed = false;
		dst->fixedPos = 0;
		// if source sample isn't empty, allocate room and copy it over (and fix it)
		if (src->length > 0 && src->dataPtrL != NULL) {
			bool sample16Bit = !!(src->flags & SAMPLE_16BIT);
			bool stereo = !!(src->flags & SAMPLE_STEREO);
			if (!allocateSmpData(dst, src->length, sample16Bit, stereo)) {
				dst->length = 0;
				return false;
			}
			// Copy L
			memcpy(dst->dataPtrL, src->dataPtrL, src->length << sample16Bit);
			// Copy R if stereo
			if (stereo && src->dataPtrR && dst->dataPtrR)
				memcpy(dst->dataPtrR, src->dataPtrR, src->length << sample16Bit);
			fixSample(dst);
		}
	}
	return true;
...
void fixSample(sample_t *s)
{
	...
	const bool stereo = !!(s->flags & SAMPLE_STEREO);
	int16_t *ptr16L = (int16_t *)s->dataPtrL;
	int16_t *ptr16R = stereo ? (int16_t *)s->dataPtrR : NULL;
	...
	*/
	if (sample16Bit)
	{
		for (int32_t i = 0; i < MAX_LEFT_TAPS; i++)
			ptr16L[i-MAX_LEFT_TAPS] = ptr16L[0];
		if (stereo && ptr16R)
		{
			for (int32_t i = 0; i < MAX_LEFT_TAPS; i++)
				ptr16R[i-MAX_LEFT_TAPS] = ptr16R[0];
		}
	}
	else
	{
		for (int32_t i = 0; i < MAX_LEFT_TAPS; i++)
			s->dataPtrL[i-MAX_LEFT_TAPS] = s->dataPtrL[0];
		if (stereo && s->dataPtrR)
		{
			for (int32_t i = 0; i < MAX_LEFT_TAPS; i++)
				s->dataPtrR[i-MAX_LEFT_TAPS] = s->dataPtrR[0];
		}
	}
	...
	if (loopType == LOOP_OFF) // no loop
	{
		if (sample16Bit)
		{
			for (int32_t i = 0; i < MAX_RIGHT_TAPS; i++)
				ptr16L[length+i] = ptr16L[length-1];
			if (stereo && ptr16R)
			{
				for (int32_t i = 0; i < MAX_RIGHT_TAPS; i++)
					ptr16R[length+i] = ptr16R[length-1];
			}
		}
		else
		{
			for (int32_t i = 0; i < MAX_RIGHT_TAPS; i++)
				s->dataPtrL[length+i] = s->dataPtrL[length-1];
			if (stereo && s->dataPtrR)
			{
				for (int32_t i = 0; i < MAX_RIGHT_TAPS; i++)
					s->dataPtrR[length+i] = s->dataPtrR[length-1];
			}
		}
	...
	if (loopType == LOOP_FWD) // forward loop
	{
		if (sample16Bit)
		{
			...
			for (int32_t i = -MAX_LEFT_TAPS; i < MAX_TAPS; i++)
			{
				...
				s->leftEdgeTapSamples16[MAX_LEFT_TAPS+i] = ptr16L[pos];
			}
			...
			for (int32_t i = 0; i < MAX_RIGHT_TAPS; i++)
			{
				s->fixedSmp[i] = ptr16L[loopEnd+i];
				ptr16L[loopEnd+i] = ptr16L[pos];

				if (++pos >= loopEnd)
					pos -= loopLength;
			}
			
			// Handle right channel for stereo samples
			if (stereo && ptr16R)
			{
				// right edge for right channel
				pos = loopStart;
				for (int32_t i = 0; i < MAX_RIGHT_TAPS; i++)
				{
					ptr16R[loopEnd+i] = ptr16R[pos];
		...
		else // 8-bit
		{
			...
			{
				pos = loopStart + myMod(i, loopLength);
				s->leftEdgeTapSamples8[MAX_LEFT_TAPS+i] = s->dataPtrL[pos];
			}
			...
			for (int32_t i = 0; i < MAX_RIGHT_TAPS; i++)
			{
				s->fixedSmp[i] = s->dataPtrL[loopEnd+i];
				s->dataPtrL[loopEnd+i] = s->dataPtrL[pos];
			...
			if (stereo && s->dataPtrR)
			{
				...
				for (int32_t i = 0; i < MAX_RIGHT_TAPS; i++)
				{
					s->dataPtrR[loopEnd+i] = s->dataPtrR[pos];
    ...
    /* If this is a stereo sample the deprecated dataPtr is NULL.
       Temporarily alias it to dataPtrL so that the legacy tap-fixer
       continues to work. */
    int8_t *origBackup = s->dataPtr;
    if (s->dataPtr == NULL)
        s->dataPtr = s->dataPtrL;
}
...
void unfixSample(sample_t *s)
{
	...
	{
		int16_t *ptr16L = (int16_t *)s->dataPtrL + s->fixedPos;
		for (int32_t i = 0; i < MAX_RIGHT_TAPS; i++)
			ptr16L[i] = s->fixedSmp[i];
		
		// Handle right channel for stereo samples
		if (stereo && s->dataPtrR)
		{
			int16_t *ptr16R = (int16_t *)s->dataPtrR + s->fixedPos;
			for (int32_t i = 0; i < MAX_RIGHT_TAPS; i++)
				ptr16R[i] = s->fixedSmp[i]; // Use same fixed samples for both channels
		}
	}
	else // 8-bit
	{
		int8_t *ptr8L = s->dataPtrL + s->fixedPos;
		for (int32_t i = 0; i < MAX_RIGHT_TAPS; i++)
			ptr8L[i] = (int8_t)s->fixedSmp[i];
		
		// Handle right channel for stereo samples
		if (stereo && s->dataPtrR)
		{
			int8_t *ptr8R = s->dataPtrR + s->fixedPos;
			for (int32_t i = 0; i < MAX_RIGHT_TAPS; i++)
				ptr8R[i] = (int8_t)s->fixedSmp[i]; // Use same fixed samples for both channels
		}
	}
...
// Refactor: stereo copy buffer support
// Replace smpCopyBuff and related vars with a struct

typedef struct {
    int8_t *L;
    int8_t *R; // NULL for mono
    int32_t size; // number of samples per channel
    bool stereo;
    int8_t bits; // 8 or 16
} smpCopyBuffer_t;

static smpCopyBuffer_t smpCopyBuffer = {NULL, NULL, 0, false, 8};
static bool smpCopyDidCopyWholeSample;

// Update getCopyBuffer to allocate both L and R for stereo
static bool getCopyBuffer(int32_t size, bool sample16Bit, bool stereo)
{
    if (smpCopyBuffer.L != NULL) free(smpCopyBuffer.L);
    if (smpCopyBuffer.R != NULL) free(smpCopyBuffer.R);
    smpCopyBuffer.L = smpCopyBuffer.R = NULL;
    smpCopyBuffer.size = 0;
    smpCopyBuffer.stereo = stereo;
    smpCopyBuffer.bits = sample16Bit ? 16 : 8;
    if (size > MAX_SAMPLE_LEN) size = MAX_SAMPLE_LEN;
    smpCopyBuffer.L = (int8_t *)malloc(size << sample16Bit);
    if (!smpCopyBuffer.L) return false;
    if (stereo) {
        smpCopyBuffer.R = (int8_t *)malloc(size << sample16Bit);
        if (!smpCopyBuffer.R) { free(smpCopyBuffer.L); smpCopyBuffer.L = NULL; return false; }
    }
    smpCopyBuffer.size = size;
    return true;
}
...
static int32_t getScaledSample(sample_t *s, int32_t index, bool rightChannel) // for sample data viewer
{
    int32_t tmp32, sample;
    const int32_t loopEnd = s->loopStart + s->loopLength;
    if (s == NULL || s->length <= 0) return 0;
    if (rightChannel && (s->flags & SAMPLE_STEREO) && s->dataPtrR != NULL) {
        if (s->flags & SAMPLE_16BIT) {
            int16_t *ptr16 = (int16_t *)s->dataPtrR;
            ...
            if (index >= s->fixedPos && index < s->fixedPos+MAX_RIGHT_TAPS && s->length > loopEnd && s->isFixed)
                ...
                tmp32 = s->dataPtrR[index];
            sample = (int8_t)((tmp32 * SAMPLE_AREA_HEIGHT) >> 8);
        }
    } else {
        if (s->flags & SAMPLE_16BIT) {
            int16_t *ptr16 = (int16_t *)s->dataPtrL;
            if (index >= s->fixedPos && index < s->fixedPos+MAX_RIGHT_TAPS && s->length > loopEnd && s->isFixed)
                tmp32 = s->fixedSmp[index-s->fixedPos];
            else
                tmp32 = ptr16[index];
...
static void writeWaveform(void)
{
    ...
    if (smpEd_ViewSize <= SAMPLE_AREA_WIDTH) // zoomed in (or 1:1)
    {
        for (int32_t x = 0; x <= SAMPLE_AREA_WIDTH; x++)
        {
            ...
            int32_t y1 = getScaledSample(s, currSmpPos, false);
            int32_t y2 = getScaledSample(s, nextSmpPos, false);
        ...
        // Optionally overlay right channel in a different color
        if ((s->flags & SAMPLE_STEREO) && s->dataPtrR != NULL) {
            for (int32_t x = 0; x <= SAMPLE_AREA_WIDTH; x++)
            {
                int32_t currSmpPos = scr2SmpPos(x+0);
                int32_t nextSmpPos = scr2SmpPos(x+1);
                ...
                int32_t x1 = smpPos2Scr(currSmpPos);
                int32_t x2 = smpPos2Scr(nextSmpPos);
                int32_t y1 = getScaledSample(s, currSmpPos, true);
                int32_t y2 = getScaledSample(s, nextSmpPos, true);
                x1 = CLAMP(x1, 0, SAMPLE_AREA_WIDTH-1);
                x2 = CLAMP(x2, 0, SAMPLE_AREA_WIDTH-1);
                if (x == SAMPLE_AREA_WIDTH)
                    x2 = SAMPLE_AREA_WIDTH-1;
                // Use a different color for right channel overlay
                sampleLine(x1, x2, y1, y2); // Optionally, change color here
            }
        }
    }
    else // zoomed out
    {
        const int32_t firstSamplePoint = getScaledSample(s, scr2SmpPos(0), false);
        ...
        for (int16_t x = 0; x < SAMPLE_AREA_WIDTH; x++)
        {
            ...
            {
                int16_t min, max;
                getSampleDataPeak(s, smpIdx, smpNum, &min, &max); // left channel
                if (x != 0)
                {
        ...
        // Optionally overlay right channel in zoomed out mode
        if ((s->flags & SAMPLE_STEREO) && s->dataPtrR != NULL) {
            int32_t firstSamplePointR = getScaledSample(s, scr2SmpPos(0), true);
            int32_t oldMinR = firstSamplePointR;
            int32_t oldMaxR = firstSamplePointR;
            for (int16_t x = 0; x < SAMPLE_AREA_WIDTH; x++)
            {
                int32_t smpIdx = scr2SmpPos(x+0);
                int32_t smpNum = scr2SmpPos(x+1) - smpIdx;
...
static bool cutRange(bool cropMode, int32_t r1, int32_t r2)
{
	...
	if (!cropMode)
	{
		if (editor.curInstr == 0 || s->dataPtrL == NULL || s->length == 0)
			return false;
		...
		{
			if (!getCopyBuffer(r2-r1, sample16Bit, stereo))
			{
				fixSample(s);
				resumeAudio();
			...
			// Copy L
			memcpy(smpCopyBuffer.L, &s->dataPtrL[r1 << sample16Bit], (r2-r1) << sample16Bit);
			// Copy R if stereo
			if (stereo && s->dataPtrR)
				memcpy(smpCopyBuffer.R, &s->dataPtrR[r1 << sample16Bit], (r2-r1) << sample16Bit);
			smpCopyBuffer.size = r2-r1;
			smpCopyBuffer.stereo = stereo;
			smpCopyBuffer.bits = sample16Bit ? 16 : 8;
		}
	}
	...
	// Move L
	memmove(&s->dataPtrL[r1 << sample16Bit], &s->dataPtrL[r2 << sample16Bit], (s->length-r2) << sample16Bit);
	// Move R if stereo
	if (stereo && s->dataPtrR)
		memmove(&s->dataPtrR[r1 << sample16Bit], &s->dataPtrR[r2 << sample16Bit], (s->length-r2) << sample16Bit);
...
static int32_t SDLCALL sampCopyThread(void *ptr)
{
	...
	if (!getCopyBuffer(smpEd_Rx2- smpEd_Rx1, sample16Bit, stereo))
	{
		okBoxThreadSafe(0, "System message", "Not enough memory!", NULL);
		return true;
	}
	...
	unfixSample(s);
	memcpy(smpCopyBuffer.L, &s->dataPtrL[smpEd_Rx1 << sample16Bit], (smpEd_Rx2-smpEd_Rx1) << sample16Bit);
	if (stereo && s->dataPtrR)
		memcpy(smpCopyBuffer.R, &s->dataPtrR[smpEd_Rx1 << sample16Bit], (smpEd_Rx2-smpEd_Rx1) << sample16Bit);
	fixSample(s);
	...
	smpCopyBuffer.size = smpEd_Rx2-smpEd_Rx1;
	smpCopyBuffer.stereo = stereo;
	smpCopyBuffer.bits = sample16Bit ? 16 : 8;
	return true;
...
{
	bool sample16Bit = (smpCopyBuffer.bits == 16);
	bool stereoDst = !!(s->flags & SAMPLE_STEREO);
	bool stereoSrc = smpCopyBuffer.stereo;
	if (!reallocateSmpData(s, smpCopyBuffer.size, sample16Bit))
	{
		okBoxThreadSafe(0, "System message", "Not enough memory!", NULL);
		return;
	}
	...
	// Paste L
	memcpy(s->dataPtrL, smpCopyBuffer.L, smpCopyBuffer.size << sample16Bit);
	// Paste R
	if (stereoDst && s->dataPtrR) {
		if (stereoSrc && smpCopyBuffer.R)
			memcpy(s->dataPtrR, smpCopyBuffer.R, smpCopyBuffer.size << sample16Bit);
		else
			memcpy(s->dataPtrR, smpCopyBuffer.L, smpCopyBuffer.size << sample16Bit);
	}
	...
	else
	{
		...
		s->finetune = 0;
		s->relativeNote = 0;
		s->flags = (smpCopyBuffer.bits == 16) ? SAMPLE_16BIT : 0;
	}
...
static int32_t SDLCALL sampPasteThread(void *ptr)
{
	...
	sample_t *s = getCurSample();
	if (smpEd_Rx2 == 0 || s == NULL || s->dataPtrL == NULL)
	{
		pasteOverwrite(s);
		return true;
	}
	...
	// set new range
	smpEd_Rx2 = smpEd_Rx1 + smpCopyBuffer.size;
...
void sampPaste(void)
{
	if (editor.curInstr == 0 || smpEd_Rx2 < smpEd_Rx1 || smpCopyBuffer.L == NULL || smpCopyBuffer.size == 0)
		return;
...
static void drawSampleOffset(void);
...
void handleSamplerRedrawing(void)
{
	...
	// Draw slice markers if they're visible
	if (getSliceMarkersVisible())
		drawSliceMarkers();

	// Draw 09xx offset indicator
	drawSampleOffset();
}
...
void handleSampleDataMouseDown(bool mouseButtonHeld)
{
	...
	// Handle slice marker interactions first
	if (!mouseButtonHeld && mouse.leftButtonPressed)
	{
		if (handleSliceMarkerMouseDown(mx, my))
		{
			mouse.lastUsedObjectType = OBJECT_SMPDATA;
			return; // Slice marker consumed the event
		}
	}
	else if (mouseButtonHeld && mouse.leftButtonPressed)
	{
		handleSliceMarkerMouseDrag(mx);
	}
...
void handleSampleEditorExtRedrawing(void)
{
	...
	hexOutBg(99,  124, PAL_FORGRND, PAL_DESKTOP, smpCopyBuffer.size, 8);
	...
	// Display 09xx offset (xx)
	uint32_t sampOffset09 = (uint32_t)(smpEd_Rx1 >> 8) & 0xFF; // same mapping as FT2 09xx (pos >> 8)
	hexOutBg(226, 122, PAL_FORGRND, PAL_DESKTOP, sampOffset09, 2);
}
...
void drawSampleEditorExt(void)
{
	...
	textOutShadow(162, 122, PAL_FORGRND, PAL_DSKTOP2, "09xx ofs.");
...
{
	// Handle slice marker mouse release
	handleSliceMarkerMouseUp();
...
// SAMPLE EDITOR OFFSET INDICATOR (09xx helper)
static void drawSampleOffset(void)
{
    if (!ui.sampleEditorShown)
        return;

    /* Convert current start marker (smpEd_Rx1) to 09xx parameter (position>>8).
       Clamp to 0x00..0xFF */
    uint32_t offset09 = (uint32_t)(smpEd_Rx1 >> 8) & 0xFF;

    char txt[16];
    sprintf(txt, "offset: %02X", offset09);

    const uint16_t txtW = textWidth(txt);
    const uint16_t x = SCREEN_W - txtW - 4; // 4-px margin from right edge
    const uint16_t y = 174 + 2;             // slightly below the waveform top

    // clear previous text background
    fillRect(x, y, txtW, FONT1_CHAR_H, PAL_DESKTOP);
