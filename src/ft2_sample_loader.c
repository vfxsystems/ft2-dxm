...
static int32_t SDLCALL loadSampleThread(void *ptr)
{
	...
	// Deep copy sample data buffers
	if (tmpSmp.dataPtrL != NULL) {
	    int32_t bytesPerSample = (tmpSmp.flags & SAMPLE_16BIT) ? 2 : 1;
	    int32_t totalBytes = tmpSmp.length * bytesPerSample;
	    s->origDataPtrL = (int8_t *)malloc(totalBytes + SAMPLE_PAD_LENGTH);
	    if (s->origDataPtrL) {
	        memcpy(s->origDataPtrL, tmpSmp.origDataPtrL, totalBytes + SAMPLE_PAD_LENGTH);
	        s->dataPtrL = s->origDataPtrL + SMP_DAT_OFFSET;
	    } else {
	        s->dataPtrL = NULL;
	    }
	}
	if ((tmpSmp.flags & SAMPLE_STEREO) && tmpSmp.dataPtrR != NULL) {
	    int32_t bytesPerSample = (tmpSmp.flags & SAMPLE_16BIT) ? 2 : 1;
	    int32_t totalBytes = tmpSmp.length * bytesPerSample;
	    s->origDataPtrR = (int8_t *)malloc(totalBytes + SAMPLE_PAD_LENGTH);
	    if (s->origDataPtrR) {
	        memcpy(s->origDataPtrR, tmpSmp.origDataPtrR, totalBytes + SAMPLE_PAD_LENGTH);
