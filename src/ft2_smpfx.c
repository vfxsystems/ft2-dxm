...
void pbSfxUndo(void)
{
	...
	if (allocateSmpData(s, s->length, !!(s->flags & SAMPLE_16BIT), !!(s->flags & SAMPLE_STEREO)))
	{
		if (s->flags & SAMPLE_16BIT) {
			if (s->flags & SAMPLE_STEREO) {
				memcpy(s->dataPtrL, sampleUndo.smpData16, s->length * sizeof(int16_t));
				memcpy(s->dataPtrR, sampleUndo.smpData16 + s->length, s->length * sizeof(int16_t));
			} else {
				memcpy(s->dataPtrL, sampleUndo.smpData16, s->length * sizeof(int16_t));
			}
		} else {
			if (s->flags & SAMPLE_STEREO) {
				memcpy(s->dataPtrL, sampleUndo.smpData8, s->length);
				memcpy(s->dataPtrR, sampleUndo.smpData8 + s->length, s->length);
			} else {
				memcpy(s->dataPtrL, sampleUndo.smpData8, s->length);
			}
		}
