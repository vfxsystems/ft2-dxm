...
static void getDexedPatchNameFromParams(const uint8_t *params, char *out, size_t outSize)
{
	...
	bool hasChar = false;
	for (int i = 0; i < 10; i++) {
		uint8_t c = params[145 + i] & 0x7F;
		if (c < 32 || c > 126) c = 32;
		name[i] = (char)c;
		if (c != 32) hasChar = true;
	}
	name[10] = '\0';
	if (!hasChar) return;
...
// Update instrument name textboxes to show TuneFish4 / Dexed status
void updateInstrumentTextBoxNames(void)
{
	// Check instruments in the current visible range (8 instruments at a time)
	for (int i = 0; i < 8; i++)
	{
		int instrNum = 1 + editor.instrBankOffset + i; // 1-based instrument number
		...
		if (instrNum >= 1 && instrNum <= MAX_INST && instr[instrNum] != NULL)
		{
			...
			else if (instr[instrNum]->useTF4)
			{
				...
				const char* presetName = ft2_synth_get_current_preset_name_for_instrument(instrNum);

				if (presetIndex >= 0 && presetName != NULL)
				{
					// Create TuneFish4 status string
					static char tf4Name[23]; // 22 chars + null terminator (textbox max)
					snprintf(tf4Name, sizeof(tf4Name), "TF4#%d:%s", instrNum, presetName);

					// Ensure it fits in the textbox (22 chars max)
					if (strlen(tf4Name) > 22)
					{
						// Truncate preset name if needed
						int presetNameLen = 22 - 13; // 22 total - "TF4#xx:" length
						if (instrNum >= 100) presetNameLen--; // Account for 3-digit instrument numbers

						snprintf(tf4Name, sizeof(tf4Name), "TF4#%d:%.6s", instrNum, presetName);
					}

					// Copy to the song instrument name
					strncpy(song.instrName[instrNum], tf4Name, 22);
					song.instrName[instrNum][22] = '\0';
				}
				else
				{
					// Fallback if preset info not available
					static char tf4Name[23];
					snprintf(tf4Name, sizeof(tf4Name), "TF4#%d", instrNum);
					strncpy(song.instrName[instrNum], tf4Name, 22);
					song.instrName[instrNum][22] = '\0';
				}
			}
			// If not TF4, but Dexed is enabled, show a Dexed identifier
			else if (instr[instrNum]->useDexed)
			{
				int presetIndex = ft2_dx_get_current_preset_for_instrument(instrNum);
				const char* presetName = (presetIndex >= 0) ? ft2_dx_get_factory_preset_name(presetIndex) : NULL;
				char patchName[16];
				getDexedPatchNameFromParams(instr[instrNum]->dxParams, patchName, sizeof(patchName));

				if (presetIndex >= 0 && presetName != NULL)
				{
					static char dxdName[23];
					snprintf(dxdName, sizeof(dxdName), "DXD#%d:%s", instrNum, presetName);
					if (strlen(dxdName) > 22)
					{
						int presetNameLen = 22 - 13;
						if (instrNum >= 100) presetNameLen--;
						snprintf(dxdName, sizeof(dxdName), "DXD#%d:%.6s", instrNum, presetName);
					}
					strncpy(song.instrName[instrNum], dxdName, 22);
					song.instrName[instrNum][22] = '\0';
				}
				else if (patchName[0] != '\0')
				{
					static char dxdName[23];
					snprintf(dxdName, sizeof(dxdName), "DXD#%d:%s", instrNum, patchName);
					if (strlen(dxdName) > 22)
					{
						int nameLen = 22 - 13;
						if (instrNum >= 100) nameLen--;
						snprintf(dxdName, sizeof(dxdName), "DXD#%d:%.6s", instrNum, patchName);
					}
					strncpy(song.instrName[instrNum], dxdName, 22);
					song.instrName[instrNum][22] = '\0';
				}
				...
				{
					char dxdName[23];
					snprintf(dxdName, sizeof(dxdName), "DXD#%d", instrNum);
					dxdName[22] = '\0';
					strncpy(song.instrName[instrNum], dxdName, 22);
					song.instrName[instrNum][22] = '\0';
				}
			}
			// If not TF4 nor Dexed, leave the existing name unchanged
		}
	}

	// Redraw the visible textboxes
	for (int i = 0; i < 8; i++)
	{
