...
int main(int argc, char *argv[])
{
	...
	printf("%zu bytes successfully written to \"%s\".\n", fileSize, fileNameOut);
	return 0;
...
static bool handleArguments(int argc, char *argv[])
{
	...
	else
	{
		...
		{
			if (!strcasecmp(argv[i], "--no-const"))
			{
				useConstFlag = false;
			}
			if (!strcasecmp(argv[i], "--terminate"))
			{
				addZeroFlag = true;
			}
			if (!strcasecmp(argv[i], "--signed"))
			{
