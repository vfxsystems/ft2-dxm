...
union V2PatchMap
{
  sU32 offsets[128];    // offsets into the full patch map block
    ...
    while (*cmd != 0xfd) // until end of stream
    {
      ...
      case 1: // Note on
        if (cmd[1] != 0) // velocity==0 is actually a note off
        {
          ...
          // and note on!
          voicesw[usevoice].note = cmd[0];
          voicesw[usevoice].velo = (sF32)cmd[1];
...
private:
  const V2Sound *getpatch(sInt pgm) const
  {
    ...
    sU32 offset = patchmap->offsets[pgm];
    if (offset < 128 * sizeof(sU32))
      offset = patchmap->offsets[0];
    if (offset < 128 * sizeof(sU32))
      return nullptr;

    return (const V2Sound *)&patchmap->raw_data[offset];
  }
  ...
  sF32 getmodsource(const V2Voice *voice, sInt chan, sInt source) const
  {
    ...
    case 8: case 9: // EG output
      COVER("MOD src EG");
      in = voice ? voice->env[source-8].out : 0.0f;
      break;

    case 10: case 11: // LFO output
      COVER("MOD src LFO");
      in = voice ? voice->lfo[source-10].out : 0.0f;
      break;

    default: // note
      COVER("MOD src note");
      in = voice ? 2.0f * (voice->note - 48.0f) : 0.0f;
      break;
    }
  ...
  void storeChanValues(sInt chan)
  {
    ...
    V2Voice *voice = nullptr;
    const sInt voiceIndex = voicemap[chan];
    if (voiceIndex >= 0 && voiceIndex < POLY && chanmap[voiceIndex] == chan)
      voice = &voicesw[voiceIndex];
  ...
  void tick()
  {
    ...
    for (sInt i=0; i < POLY; i++)
    {
      ...
      {
        if (voicemap[chanmap[i]] == i)
          voicemap[chanmap[i]] = -1;
        chanmap[i] = -1;
      }
    }
